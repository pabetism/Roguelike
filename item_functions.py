...
def cast_fireball(map_x, *args, **kwargs):
    entities = kwargs.get('entities')
    fov_map = kwargs.get('fov_map')
    damage = kwargs.get('damage')
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x - map_x, target_y -t):
    ...
    for entity in entities:
        if entity.distance(target_x - map_x, target_y - 1) <= radius and entity.fighter:
